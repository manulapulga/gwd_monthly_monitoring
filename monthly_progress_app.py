...
def render_data_entry_form(entry_data=None, is_editable=True):
    ...
    with col3:
        if is_editable:
            reporting_date = st.date_input("Reporting Date", value=reporting_date)
        else:
            # Display as read-only
    ...
    with st.container(border=True):
        ...
        with col1:
            if is_editable:
                ...
                # Display as read-only
                st.markdown(f"**Name of Reporting Officer:** {form_data.get('officer_name', '')}")
                st.markdown(f"**Designation:** {form_data.get('designation', '')}")
        ...
        with col2:
            if is_editable:
                ...
                # Display as read-only
                st.markdown(f"**Contact Number:** {form_data.get('contact', '')}")
            ...
            for field in details['fields']:
                with cols[col_index % 2]:
                    ...
                    if is_editable:
                        ...
                        # Display as read-only
                        display_value = existing_value if existing_value != '' else 'Not provided'
                        if field['type'] == 'number':
                            display_value = f"{display_value} {field.get('unit', '')}"
...
def district_dashboard():
    ...
    with st.sidebar:
        ...
        # Clear edit/view states when switching tabs
        if selected_tab != "📝 New Entry" or (selected_tab == "📝 New Entry" and not st.session_state.editing_entry):
            st.session_state.editing_entry = None
            st.session_state.viewing_entry = None
        ...
        if not all_data:
            ...
            for entry in filtered_data:
                with st.container(border=True):
                    ...
                    with col3:
                        submitted_date = entry.get('submitted_at', 'N/A')
                        if submitted_date != 'N/A':
                            submitted_date = submitted_date.strftime('%Y-%m-%d %H:%M')
                        st.caption(f"Submitted: {submitted_date}")
                    
                    with col4:
                        modified_date = entry.get('last_modified', 'N/A')
                    ...
                    with col6:
                        ...
                        with action_col2:
                            details_key = f"details_{entry['district']}_{entry['year']}_{entry['month']}"
